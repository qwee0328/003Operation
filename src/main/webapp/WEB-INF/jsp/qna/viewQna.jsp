			...
			<div class="boardPost">
				...
				<c:choose>
					<c:when test="${empty post.answer}">
						<c:choose>
							<c:when
								test="${pageContext.request.userPrincipal.authorities eq '[ROLE_ADMIN]'}">
											...
											<div class="fileNameList d-flex">
												<c:choose>
													<c:when test="${not empty post.answer.file_names}">
														<script>
															// 기존 파일 목록
															let fileNames = `${post.answer.file_names}`
																	.split(",");
															let fileIds = `${post.answer.file_ids}`
																	.split(",");
							...
							<c:when
								test="${pageContext.request.userPrincipal.authorities != '[ROLE_ADMIN]'}">
								<div class="boardPost__guide">
									<div class="answerTitle">QNA 게시글 답변</div>
									<div class="answerConf">등록된 답변이 아직 없습니다.</div>
								</div>
