		...
		<div class="boardPost_guide">
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<script>
						for (let i = 0; i < fileNames.length; i++) {
							let fileNameTag = $("<div>").attr("class",
									"fileNameTag d-flex");
							let fileName = $("<div>").attr("class", "fileName")
									.text(fileNames[i]);
							let fileIcon = $("<div>")
									.attr("class", "ml5")
									.html(
											"<i class='fa-solid fa-xmark deleteFileBtn' data-seq="+fileIds[i]+"></i>")
							$(".fileNameList").append(
									fileNameTag.append(fileName).append(
											fileIcon))
						}
					</script>
				</c:otherwise>
...
<!-- 	</div> -->
	<!-- 
	<input type="file" class="qnaAnswerWrite__fileInput" id="fileInput"
		multiple>
	</div>
	 -->
	<%@ include file="/WEB-INF/jsp/commons/summernote.jsp"%>
