...
function postLoad(cpage){
	$.ajax({
		url:"/reply/mypage/selectMyReply",
		type:"post",
		data:{cpage:cpage}
...
function postPrint(result){
	...
	for(let i=0; i<post.length; i++){
		...
		board__post.append(post__chkBox.append(chkBox)).append(post__seq).append(post__tacCover).append(post__writeDate).append(post__recomCount);
...
$(document).on("click",".board__selectDelete",function(){
	if(confirm("정말로 삭제하시겠습니까?")){
		...
		$.ajax({
			url:"/reply/mypage/deleteSelectReply",
			type:"post",
            data: formData,
			contentType: false,
			processData: false
...
function search(cpage){
	...
	$.ajax({
		url:"/reply/mypage/searchMyReply",
		data:{
			select:select,
			keyword:keyword,
			cpage:cpage
